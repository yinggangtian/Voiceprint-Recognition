...
def plot_loss_acc(file=c.PRE_CHECKPOINT_FOLDER+'/test_loss_acc.txt'):
    ...
    plt.xlabel("Steps")
    plt.ylabel("Accuracy ")
    plt.savefig('figs/acc.png', dpi=300)
    plt.show()
...
def plot_acc(file=c.CHECKPOINT_FOLDER+'/acc_eer.txt'):
    ...
    p4, = plt.plot(step, mov_eer, color='green', label='Moving_Average_EER')
    plt.xlabel("Steps")
    plt.ylabel("")
    plt.legend(handles=[p1,p2,p3,p4],labels=['F-measure','EER','Accuracy','moving_eer'],loc='best')
    plt.savefig('figs/acc_eer.png', dpi=300)
    plt.show()
...
if __name__ == "__main__":
    plot_loss_acc(file='gru_checkpoints/losses_gru.txt')
    plot_loss(file='checkpoints/losses.txt')
    plot_acc(file='checkpoints/acc_eer.txt')