...
def eval_model(model,train_batch_size=c.BATCH_SIZE * c.TRIPLET_PER_BATCH, test_dir= c.TEST_DIR, check_partial=False, gru_model=None):
    ...
    fm, tpr, acc, eer, frr, far = evaluate(y_pred, y_true)
    return fm, tpr, acc, eer, frr, far
...
if __name__ == '__main__':
    ...
    #MARK 使用gru模型
    fm, tpr, acc, eer, frr, far = eval_model(gru_model, check_partial=True,gru_model=gru_model)
    print("f-measure = {0}, true positive rate = {1}, accuracy = {2}, equal error rate = {3}, frr={4}, far={5}".format(fm, tpr, acc, eer, frr, far))
