...
'''
#=======================================================================================
def recurrent_model_softmax(input_shape=(NUM_FRAMES, 64, 1),
                    batch_size=BATCH_SIZE * TRIPLET_PER_BATCH ,num_frames=NUM_FRAMES, num_spks=NUM_SPEAKERS):
    inputs = Input(shape=input_shape)
    #x = Permute((2,1))(inputs)
    x = Conv2D(64,kernel_size=5,strides=2,padding='same',kernel_initializer='glorot_uniform',kernel_regularizer=regularizers.l2(l=0.0001))(inputs)
    # x = BatchNormalization()(x)  #shape = (BATCH_SIZE , num_frames/2, 64/2, 64)
...
def recurrent_model_sigmoid_cross_entropy(input_shape=(NUM_FRAMES, 64, 1),
                    batch_size=BATCH_SIZE * TRIPLET_PER_BATCH ,num_frames=NUM_FRAMES, num_spks=NUM_SPEAKERS):
    '''
...
def recurrent_model_cross_entropy(input_shape=(NUM_FRAMES, 64, 1),
                    batch_size=BATCH_SIZE * TRIPLET_PER_BATCH ,num_frames=NUM_FRAMES, num_spks=NUM_SPEAKERS):
    '''
